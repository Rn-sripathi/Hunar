"""Coerces Hunar's extracted answers into typed values.

Hunar returns every field as a string, whatever type the schema declared.
A field declared ``"boolean"`` comes back as ``"Yes"``, ``"true"``,
``"Haan"`` or ``"yes sir"``. A field declared ``"number"`` comes back as
``"3"``, ``"3 years"`` or ``"around three"``. Nothing downstream can sort,
filter or score those without a coercion step, so this module is required
rather than convenient.

Three principles govern it:

* **Never discard the original.** The raw string is stored alongside the
  coerced value and shown in the UI, so a recruiter can always read what
  the candidate actually said. A coercion bug then costs a misread
  column, not a lost answer.
* **Unparseable is not false.** A value this module cannot understand
  becomes ``None``, never ``False`` or ``0``. Treating "I'm not sure" as a
  no would silently reject a candidate on a parsing failure.
* **Answer in the candidate's language.** Screening runs in Hindi, Tamil
...
from app.hiring.schemas import FieldSpec
...
def normalize_result(
    ...
    """Coerce a whole result payload against a job's field specification.
